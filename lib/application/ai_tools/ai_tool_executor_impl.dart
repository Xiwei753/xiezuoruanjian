...
class AIToolExecutorImpl implements IAIToolExecutor, AIToolRegistry {
  ...
  AIToolExecutorImpl(this._registeredTools, this._workspaceRoot, this._dbHelper, this._storageService);
  ...
  Future<AIToolResult> executeTool(AIToolCall call) async {
    ...
    if (toolDef == null) {
      return AIToolResult(toolCallId: call.id, name: call.name, result: {'error': 'Tool not found'}, isError: true);
    }

    if (toolDef.riskLevel == AIRiskLevel.dangerous) {
      return AIToolResult(toolCallId: call.id, name: call.name, result: {'error': 'Tool execution blocked: DANGEROUS risk level'}, isError: true);
    }
    ...
    try {
      ...
      return AIToolResult(toolCallId: call.id, name: call.name, result: {'error': e.toString()}, isError: true);
    }
  }
  ...
  Future<Map<String, dynamic>> _dispatch(AIToolCall call) async {
    switch (call.name) {
      ...
      case 'save_chapter_summary':
        ...
        final aiPath = p.join(_workspaceRoot, 'app-meta', 'ai', 'summaries', '${chapterId}_summary.json');
        await _storageService.atomicWrite(aiPath, jsonEncode({'summary': summary}));
        return {'status': 'saved', 'path': aiPath};

      case 'save_character_extraction':
        final projectId = call.arguments['projectId'] as String;
        final aiPath = p.join(_workspaceRoot, 'projects', projectId, 'characters', 'ai_extract_${DateTime.now().millisecondsSinceEpoch}.json');
        await _storageService.atomicWrite(aiPath, jsonEncode(call.arguments['characters']));
        return {'status': 'saved', 'path': aiPath};
