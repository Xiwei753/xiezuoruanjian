...
def build_repo_sets(repo_root, exclude_dirs):
    all_repo_paths = set()
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        for d in dirs:
            ...
            rel_p = os.path.relpath(full_p, repo_root)
            all_repo_paths.add(rel_p.replace(os.sep, '/'))
    return all_repo_paths, all_repo_basenames

def check_standard_links(content_no_code, rel_md_path, root, repo_root):
        ...
        if target_path:
            ...
            if target_path.startswith(repo_root):
                if not os.path.exists(target_path):
                    ...
                    print()
                    broken_count += 1
    return checked_links_count, broken_count

def check_plain_text_paths(content_no_code, rel_md_path, all_repo_paths):
    ...
    for line in content_no_code.split('\n'):
        ...
        for p_match in path_matches:
            ...
            if normalized_path not in all_repo_paths:
                ...
                print()
                broken_count += 1
    return checked_paths_count, broken_count

def check_plain_text_filenames(content_no_code, rel_md_path, all_repo_basenames):
    ...
    for line in content_no_code.split('\n'):
        ...
        for f_match in file_matches:
            ...
            if f_match not in all_repo_basenames:
                ...
                print()
                broken_count += 1
    return checked_files_count, broken_count

def check_markdown_links_and_paths(repo_root, exclude_dirs, all_repo_paths, all_repo_basenames):
    broken_count = 0
    checked_links_count = 0
    checked_paths_count = 0
    checked_files_count = 0
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        for file in files:
            ...
            links_c, links_b = check_standard_links(content_no_code, rel_md_path, root, repo_root)
            paths_c, paths_b = check_plain_text_paths(content_no_code, rel_md_path, all_repo_paths)
            files_c, files_b = check_plain_text_filenames(content_no_code, rel_md_path, all_repo_basenames)
            
            checked_links_count += links_c
            broken_count += links_b
            checked_paths_count += paths_c
            broken_count += paths_b
            checked_files_count += files_c
            broken_count += files_b

    return broken_count, checked_links_count, checked_paths_count, checked_files_count

def check_forbidden_phrases(repo_root, exclude_dirs):
            ...
            for line in content_no_code.split('\n'):
                for phrase in FORBIDDEN_PHRASES:
                    if phrase in line:
                        ...
                        print()
    return forbidden_phrase_count

def check_forbidden_code_patterns(repo_root, exclude_dirs):
            ...
            for line_no, line in enumerate(content.split('\n'), 1):
                for pattern in patterns:
                    if pattern in line:
                        ...
                        print(f"  Line: '{line.strip()}'")
                        print()
    return forbidden_code_pattern_count
...
def check_links():
    ...
    all_repo_paths, all_repo_basenames = build_repo_sets(repo_root, exclude_dirs)

    broken_count, checked_links_count, checked_paths_count, checked_files_count = check_markdown_links_and_paths(
        repo_root, exclude_dirs, all_repo_paths, all_repo_basenames)

    forbidden_phrase_count = check_forbidden_phrases(repo_root, exclude_dirs)

    forbidden_code_pattern_count = check_forbidden_code_patterns(repo_root, exclude_dirs)
