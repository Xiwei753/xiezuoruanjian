...
| **自研写作区路线** | Linux_qt 自研写作区唯一主路径：`SujianEditorItem(QQuickItem)` + `QTextLayout/QTextLine` + `QImage static texture` + `QSGImageNode` + QML Rectangle cursor + Rust `AnimationCoordinator`/`RenderPlan`/`VisualPayload`。 |
...
- Linux_qt 自研写作区当前唯一主路径：`SujianEditorItem(QQuickItem)` + `QTextLayout/QTextLine` + `QImage static texture` + `QSGImageNode` + QML Rectangle cursor + Rust `AnimationCoordinator`/`RenderPlan`/`VisualPayload`。
- 排查入口：`apps/Linux_qt/src/sujian_editor_item/*`、`apps/Linux_qt/src/editor/layout.rs`、`apps/Linux_qt/src/editor/renderer.rs`、`apps/Linux_qt/src/editor/scene_graph.rs`、`apps/Linux_qt/qml/WritingWorkspace.qml`。
