...
- **禁止平台绕道**：平台端严禁越过 Command 机制直接修改 workspace 下的任何文件（包括但不限于 `mind_map/index.json`（LEGACY）、settings文件、`.git/` 下的文件）。
...
- ❌ **错误做法**：
  Android 通过 JNI 调起 `createFolder`，然后在 Android 侧向文件夹里写入 `.nomedia`，接着更新 SharedPreferences 中记录的“最近项目”；同时 Desktop 侧由 backend 创建文件夹后，直接生成了一个默认的 settings文件 并调用 QML 发送状态刷新。
...
-  **正确做法**：
  ...
  Core 将数据写入到工作区指定的 settings文件 中，并在写入成功时广播 `SettingsSaved` 事件。平台端监听该事件，读取最新的有效设置并更新编辑器字体大小。
