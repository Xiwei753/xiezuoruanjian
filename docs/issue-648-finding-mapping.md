...
## 表 A：Code Quality Standard Findings（原 22 条）

当前无法从 GitHub Code Quality API 取得原 22 条 Standard findings 的明细。

**事实陈述**：Code Quality API 本次请求返回 404，未能通过该请求取得 Standard findings。不能仅凭该响应判断仓库是否启用了 Code Quality；还可能涉及 API 版本、权限、资源可见性等条件。

**后续行动**：需要从 Security & Quality 的 Standard findings 页面或对应 `github-code-quality[bot]` 结果取得真实规则/路径后再填入下表。在取得真实明细前，不猜测、不凑数。

| 序号 | finding/rule | severity | 原文件路径 | 当前文件路径 | 处理方式 | 对应提交 | 当前结论 |
|------|-------------|----------|-----------|-------------|----------|---------|---------|
| 1-22 | 待填入 | 待填入 | 待填入 | 待填入 | 待填入 | 待填入 | 待填入 |
...
1. **表 A（22 条 Standard findings）**：当前无法从 Code Quality API 取得明细（返回 404），不能仅凭该响应判断仓库是否启用了 Code Quality。需从 Security & Quality 页面或 `github-code-quality[bot]` 结果取得真实规则/路径后再填入。在取得真实明细前，不猜测、不凑数。
...
4. **表 B.3 Dismissed（8 条）**：8 条 `rust/access-invalid-pointer` 告警均为 qmetaobject FFI 边界代码，涉及 `Rc<RefCell<AppBackend>>` 安全借用、`QPointer` 弱引用跨线程回调和 `QObjectPinned` 注册。逐条分析确认：指针由 `Rc`/`QObjectBox` 持有，生命周期由引用计数和 `RefCell` 借用检查保证，不会悬空/越界。GitHub 上 dismissal reason 均为 `false positive`。

**不声称"所有 22 个 finding 都已映射"**：表 A 的 22 条 Standard findings 明细尚未取得，表 B 的 15 条 Open alert 尚未在默认分支上确认关闭。
