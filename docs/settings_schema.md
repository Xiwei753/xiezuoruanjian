...
- `desktopSidebarWidth`（浮点数，Linux 和 Windows 客户端侧边栏宽度像素，默认 240.0；兼容读取 `linux_qt_sidebar_width`、`desktop_sidebar_width`、`linux_sidebar_width`）
- `desktopEditorWidth`（浮点数，Linux 和 Windows 客户端编辑器宽度像素，默认 0.0；兼容读取 `linux_qt_editor_width`、`desktop_editor_width`）
