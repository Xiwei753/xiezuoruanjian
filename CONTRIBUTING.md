...
- **客户端做薄**：客户端（Android / Linux_qt / Windows / HarmonyOS）只负责 UI 渲染、导航、输入法交互和主题。不允许在客户端中实现业务逻辑。
...
- **Thin clients**: Clients (Android / Linux_qt / Windows / HarmonyOS) are only responsible for UI rendering, navigation, IME interaction, and theming. Implementing business logic in clients is not allowed.
