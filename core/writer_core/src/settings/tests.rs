...
fn test_settings() {
    ...
    // Issue #705: LocalSettings 已删除 theme_mode 字段,运行时只认 appearance_mode。
    assert_eq!(local.appearance_mode.as_str(), "system");
    ...
    local.appearance_mode = "light".to_string();
    ...
    assert_eq!(loaded_local.appearance_mode, "light");
