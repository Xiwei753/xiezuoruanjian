...
fn test_reorder_volumes_mismatch_error() {
    ...
    let result = reorder_volumes(&data_root.join("projects").join(&project.id), &ordered_ids);
    match result {
    ...
    let result = reorder_volumes(&data_root.join("projects").join(&project.id), &extra_ids);
    match result {
        Err(crate::error::Error::Other(msg)) => {
            assert_eq!(msg, "Invalid ordered_ids for reorder")
        }
...
fn test_reorder_volumes_success() {
    ...
    let new_volumes = list_volumes(&data_root.join("projects").join(&project.id)).unwrap();
