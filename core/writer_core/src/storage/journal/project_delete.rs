...
//! - `TombstoneWritten`: tombstone 已生成并持久化
//! - `StarMapsUnbound`: starmap 解绑已完成
//! - `HistoryRecorded`: workspace Git history 已记录
...
impl ProjectDeleteTransaction {
    ...
    ///
    /// 成功后推进 phase 到 `TombstoneWritten`，
    ...
    pub fn write_tombstone(&mut self) -> Result<()> {
        ...
        // tombstone 已写盘，推进 phase 到 TombstoneWritten。
...
/// API/bootstrap 记 history 成功后推进 journal。
///
