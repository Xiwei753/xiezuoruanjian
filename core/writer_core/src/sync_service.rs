...
fn get_user_friendly_error(err: &str) -> String {
    ...
    if e.contains("failed to resolve address") || e.contains("no address associated with hostname") || e.contains("could not resolve host") || e.contains("name resolution") {
    ...
    if e.contains("ssl") || e.contains("certificate") {
        return "SSL 证书或网络错误。请检查您的网络环境、代理/VPN 设置或系统时间是否正确。".to_string();
    }
    ...
    pub fn perform_sync(
        ...
        if let Err(e) = backend.pull(workspace_path, &config.branch, auth.as_ref()) {
            ...
            return Ok(SyncResult::error(
                    ...
                    Some(get_user_friendly_error(&(format!("Pull failed: {}", e)).to_string())),
                    format!("Pull failed: {}", e),
                ));
        }
        // Get Plan
        ...
        if !paths_to_stage.is_empty() {
            if let Err(e) = backend.stage_paths(workspace_path, &paths_to_stage) {
                return Ok(SyncResult::error(
                    ...
                    result.first_sync_mode,
                    Some(get_user_friendly_error(&(format!("Stage failed: {}", e)).to_string())),
        ...
        if !actual_staged.is_empty() {
            match backend.commit(workspace_path, "Auto sync") {
                ...
                Err(e) => {
                    ...
                    result.first_sync_mode,
                    Some(get_user_friendly_error(&(format!("Commit failed: {}", e)).to_string())),
            ...
            if let Err(e) = backend.push(workspace_path, &config.branch, auth.as_ref()) {
                return Ok(SyncResult::error(
                    ...
                    result.first_sync_mode,
                    Some(get_user_friendly_error(&(format!("Push failed: {}", e)).to_string())),
