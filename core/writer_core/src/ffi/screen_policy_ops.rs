...
use crate::screen_policy::{ScreenRole, resolve_screen_policy};
...
pub unsafe extern "C" fn writer_core_resolve_screen_policy(
    ...
    let role_str = match c_str_to_rust(screen_role_json) {
        Ok(s) => s,
        Err(e) => {
            return err_json("INVALID_INPUT", &format!("screen_role_json is null or invalid UTF-8: {}", e));
        }
    };
    ...
    let screen_role: ScreenRole = match serde_json::from_str(&role_str) {
        Ok(r) => r,
        Err(e) => {
            return err_json("PARSE_ERROR", &format!("Failed to parse ScreenRole JSON: {}", e));
        }
    };
    ...
    let mode_str = match c_str_to_rust(shell_mode_json) {
        Ok(s) => s,
        Err(e) => {
            return err_json("INVALID_INPUT", &format!("shell_mode_json is null or invalid UTF-8: {}", e));
        }
    };
    ...
    let shell_mode: ShellMode = match serde_json::from_str(&mode_str) {
        Ok(m) => m,
        Err(e) => {
            return err_json("PARSE_ERROR", &format!("Failed to parse ShellMode JSON: {}", e));
        }
    };
