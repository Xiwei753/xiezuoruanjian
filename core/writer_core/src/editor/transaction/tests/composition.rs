...
fn composition_commit_transaction_visual_same_no_repeat() {
    ...
    let tx =
        engine.composition_commit_or_cancel_transaction("hello", "hello world", comp_rev, true);
    assert!(tx.is_commit);
...
fn composition_update_does_not_change_committed_text() {
    let mut engine = EditorEngine::new();
    let tx = engine.composition_update_transaction("original", Some((0, 4)), "orig", "new_text");
...
fn commit_same_visual_text_no_repeat_animation() {
    ...
    assert!(
        tx.is_visual_same,
        "Same visual text must not repeat animation"
    );
}
...
fn build_virtual_text_zero_length_replace_preserves_text_after_cursor() {
    ...
    assert_eq!(
        vt, "你好abc世界",
        "Zero-length replace must preserve text after cursor"
    );
}
...
fn composition_session_zero_length_replace_by_default() {
    let session = CompositionSession::new(1, 10, "你好世界".to_string(), "你好".len());
    assert_eq!(
        session.replace_start,
        Utf8ByteOffset::unchecked("你好".len())
    );
    assert_eq!(
        session.replace_end_exclusive,
        Utf8ByteOffset::unchecked("你好".len())
    );
}
...
fn composition_session_update_preedit_preserves_replace_range() {
    let mut session = CompositionSession::new(1, 10, "你好世界".to_string(), "你好".len());
...
fn composition_session_set_composing_region() {
    let mut session = CompositionSession::new(1, 10, "你好世界".to_string(), 0);
...
fn composition_session_update_creates_revision_chain() {
    ...
    assert!(
        rev2.offset_map_from_previous.is_some(),
        "Second revision must have offset map"
    );
...
fn composition_session_does_not_modify_committed_text() {
    let mut session = CompositionSession::new(1, 10, "original".to_string(), 4);
...
fn composition_session_clear_resets_preedit() {
    let mut session = CompositionSession::new(1, 10, "hello".to_string(), 5);
...
fn composition_visual_revision_from_previous_chains_correctly() {
    ...
    assert_eq!(
        rev2.composition_replace_range,
        Utf8ByteRange::from_values(6, 6)
    );
...
fn offset_map_suffix_shifted() {
    ...
    let suffix = map
        .entries
        .iter()
        .find(|e| e.kind == OffsetMapKind::Shifted);
...
fn offset_map_map_old_to_new_no_mapping_for_middle() {
    ...
    assert!(
        map.map_old_to_new(1).is_none(),
        "Middle changed region has no mapping"
    );
}
...
fn snapshot_owner_serializes_camel_case() {
    ...
    let json2 =
        serde_json::to_string(&SnapshotOwner::OwnedByTransaction { transaction_id: 42 }).unwrap();
...
fn snapshot_owner_equality() {
    assert_eq!(
        SnapshotOwner::OwnedBySession { session_id: 1 },
        SnapshotOwner::OwnedBySession { session_id: 1 }
    );
    assert_eq!(
...
fn commit_with_replace_range_replaces_correctly() {
    ...
    assert!(
        tx.is_visual_same,
        "Same visual text on commit with replace range"
    );
}
...
fn cancel_with_replace_range_restores_committed() {
    ...
    assert!(!tx.is_commit);
    assert!(
        !tx.visual_class_kinds.is_empty(),
        "Cancel must produce visual classifications"
    );
}
...
fn composition_session_full_lifecycle() {
    let mut session = CompositionSession::new(1, 100, "你好世界".to_string(), "你好".len());

    let rev1 = session.update_preedit("n".to_string(), 1);
    assert_eq!(rev1.virtual_text, "你好n世界");
    assert_eq!(
        rev1.composition_replace_range,
        Utf8ByteRange::from_values(6, 6)
    );
    ...
    assert_eq!(
        rev3.composition_replace_range,
        Utf8ByteRange::from_values(6, 6)
    );
}
...
fn composition_session_with_composing_region() {
    let mut session =
        CompositionSession::new_with_replace_range(1, 100, "你好世界".to_string(), 3, 9);
    let rev = session.update_preedit("abc".to_string(), 3);
    assert_eq!(rev.virtual_text, "你abc界");
    assert_eq!(
        rev.composition_replace_range,
        Utf8ByteRange::from_values(3, 9)
    );
}
...
fn composition_session_virtual_text_nonzero_replace() {
    let mut session =
        CompositionSession::new_with_replace_range(1, 1, "你好世界".to_string(), 3, 6);
    session.update_preedit("abc".to_string(), 0);
...
fn composition_session_preedit_byte_range_in_virtual_text() {
    ...
    assert_eq!(
        ...
        "preedit range in virtualText differs from replaceRange"
    );
}
...
fn composition_session_clear_resets_last_submitted_generation() {
    ...
    assert!(session.current_visual_revision.is_none());
    assert_eq!(
        session.last_submitted_generation,
        EditorSessionGeneration::new(0)
    );
}
