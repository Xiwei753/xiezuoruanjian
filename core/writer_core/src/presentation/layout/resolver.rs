...
//! `WorkspaceLayoutMode` 只有 `SinglePane` / `Workbench` 两个产品语义变体（ 验收点 1），
//! 不再输出旧的 `ListDetail` / `ThreePane`。
//!
//! # Workbench 布局计划（  第 1-2 步，）
//!
...
/// ```
///
/// `occlusions` 描述窗口中被系统 UI 遮挡的区域（ 验收点 5），
/// 默认为空 Vec（无遮挡）。
...
/// 根据宽度 class 与高度 class 推导壳层与工作区布局模式（ 验收点 1）。
///
/// 关键规则：
