...
//! `WorkspaceLayoutMode` 只有 `SinglePane` / `Workbench` 两个产品语义变体，
//! 不再输出旧的 `ListDetail` / `ThreePane`。
//!
//! # Workbench 布局计划（第 1-2 步，问题 2/3）
//!
...
//!
//! （02:59:39Z 版）：当 free regions 在合理最小尺寸下已放不下完整
...
/// 工作台布局计划 — `resolve_workbench_layout` 的输出（第 1 步）。
///
...
/// 根据宽度 class 与高度 class 推导壳层与工作区布局模式。
///
/// 关键规则：
...
///
/// 第 1 步：删除 `workbench_occlusion` 字段，
