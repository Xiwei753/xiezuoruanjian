...
fn upsert_edge_existing_marks_dirty_graph_meta() {
    ...
    assert_eq!(
        eri.to, "n3",
        "disk relation index should reflect updated endpoint"
    );
}
#[test]
fn update_edge_endpoint_marks_dirty_graph_meta() {
    ...
    assert!(
        store.dirty_graph_meta,
        "update_edge changing from_target should mark dirty_graph_meta"
    );
    ...
    assert_eq!(
        eri.from, "n3",
        "disk relation index should reflect updated from endpoint"
    );
}
#[test]
fn update_embed_host_marks_dirty_graph_meta() {
    ...
    use crate::starmap::types::{
        StarMapEmbed, StarMapEmbedPatch, StarMapEmbedPlacement, StarMapEmbedViewport,
    };
    ...
    assert!(
        store.dirty_graph_meta,
        "update_embed changing source_node_id should mark dirty_graph_meta"
    );
    ...
    assert_eq!(
        ehi.host_node_id, "n2",
        "disk host index should reflect updated host"
    );
}
#[test]
...
fn update_edge_endpoint_marks_dirty_and_updates_index() {
    ...
    let patch = StarMapEdgePatch {
        ...
        from_target: None,
        to_target: None,
        from_endpoint: Some(Some(StarMapEdgeEndpoint::Node {
    ...
    assert!(
        store.is_dirty(),
        "update_edge with endpoint change should mark dirty"
    );
    let eri = store
        .graph_meta
        .as_ref()
    ...
    assert!(
        ...
        "index should reflect updated from_endpoint"
    );
}
#[test]
fn update_embed_host_endpoint_marks_dirty_and_updates_index() {
    ...
    let patch = StarMapEmbedPatch {
        ...
        source_node_id: None,
        host_anchor: None,
        host_endpoint: Some(Some(StarMapEndpoint::Anchor {
            node_id: "n1".to_string(),
    ...
    assert!(
        store.is_dirty(),
        "update_embed with host_endpoint change should mark dirty"
    );
    let ehi = store
        .graph_meta
        .as_ref()
    ...
    assert!(
        ...
        "index should reflect updated host_endpoint"
    );
}
#[test]
fn add_link_updates_graph_meta_link_ids() {
    ...
    assert!(
        store.graph_meta.is_some(),
        "add_link must initialize graph_meta"
    );
...
fn hyperlink_add_update_delete_round_trip() {
    ...
    assert!(
        store.dirty_graph_meta,
        "add_hyperlink must mark dirty_graph_meta"
    );
    assert!(store
        .graph_meta
        .as_ref()
    ...
    assert_eq!(
        store2.get_hyperlink("hl1").unwrap().label.as_deref(),
        Some("TestHL")
    );

    let updated = store2
        .update_hyperlink("hl1", Some("UpdatedHL"), None)
        .unwrap();
    ...
    assert!(!store3
        ...
        .contains(&"hl1".to_string()));
    assert!(
        store3.dirty_graph_meta,
        "delete_hyperlink must mark dirty_graph_meta"
    );
...
fn delete_node_cascades_to_links_and_hyperlinks() {
    ...
    assert!(
        store2.get_link("l1").is_none(),
        "delete_node must cascade remove link"
    );
    assert!(
        store2.get_hyperlink("hl1").is_none(),
        "delete_node must cascade remove hyperlink"
    );
