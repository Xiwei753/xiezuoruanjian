    ...
    pub(in crate::starmap::store) fn try_load_embed(
        ...
        let (path, from_flat) = if bucket_path.exists() {
            ...
            self.recovery_log.push(LoadDiagnostic {
                ...
                detail: format!(
                    "embed file not found: tried {} and {}",
                    bucket_path.display(),
                    flat_path.display()
                ),
            });
    ...
    pub(in crate::starmap::store) fn try_load_hyperlink(
        ...
        let (path, from_flat) = if bucket_path.exists() {
            ...
            self.recovery_log.push(LoadDiagnostic {
                ...
                detail: format!(
                    "hyperlink file not found: tried {} and {}",
                    bucket_path.display(),
                    flat_path.display()
                ),
            });
        ...
        if let Some(layout) = package_storage::load_legacy_layout(&dir) {
            if package_storage::save_layout_sharded(&dir, &layout).is_ok() {
                ...
                self.record_migration(
                    "layout_sharded",
                    "migrated legacy default.json to sharded format",
                );
            }
            return Some(layout);
        }
