...
如果你需要下载 Android APK 进行手机端测试，你可以通过 GitHub Actions 直接构建并下载：

1. 打开本项目的 GitHub 仓库页面。
2. 点击上方的 **Actions** 标签。
3. 在左侧的工作流列表中，选择 **Android Debug Build**。
4. 点击右侧的 **Run workflow** 按钮，或者等待代码 `push` 后自动触发构建。
5. 等待构建运行完成（状态变为绿色勾号）。
6. 点进该次运行的详情页，在最下方的 **Artifacts** 区域，下载名为 `writer-app-debug-apk` 的压缩包。
7. 解压下载的压缩包，即可得到 `app-debug.apk` 文件。
8. 将该 APK 传到你的 Android 手机上安装并测试。

**说明：**
- 这是一个 **Debug APK**，没有正式发布签名，仅供本地测试和早期体验使用。
- 安装时，你的手机可能会提示“未知来源应用”，请允许安装以继续。
