...
public sealed class WriterCoreBridge
{
    ...
    public int InitWorkspace(string path)
    {
        ...
        try { return writer_core_init(pathPtr); }
        ...
        try
        {
            ...
            if (env.Data?.ValueKind == JsonValueKind.Object && env.Data.Value.TryGetProperty("projects", out var arr))
            {
                foreach (var p in arr.EnumerateArray())
                    projects.Add(new ProjectSummary(p.GetProperty("id").GetString() ?? "", p.GetProperty("title").GetString() ?? ""));
            }
    ...
    public Task<IReadOnlyList<ProjectSummary>> ListProjectsAsync()
    {
        ...
        if (env.Data?.ValueKind == JsonValueKind.Array)
        {
            foreach (var p in env.Data.Value.EnumerateArray())
                list.Add(new ProjectSummary(p.GetProperty("id").GetString() ?? "", p.GetProperty("title").GetString() ?? ""));
        }
        ...
        try
        {
            ...
            if (env.Data?.ValueKind == JsonValueKind.Array)
            {
                foreach (var v in env.Data.Value.EnumerateArray())
                    list.Add(new VolumeSummary(v.GetProperty("id").GetString() ?? "", v.GetProperty("title").GetString() ?? ""));
            }
        ...
        try
        {
            ...
            if (env.Data?.ValueKind == JsonValueKind.Array)
            {
                foreach (var c in env.Data.Value.EnumerateArray())
                    list.Add(new ChapterSummary(c.GetProperty("id").GetString() ?? "", c.GetProperty("title").GetString() ?? ""));
            }
            return Task.FromResult<IReadOnlyList<ChapterSummary>>(list);
        }
        finally { Marshal.FreeHGlobal(pidPtr); Marshal.FreeHGlobal(vidPtr); }
    }
    ...
    public Task<string> OpenChapterAsync(string projectId, string volumeId, string chapterId)
    {
        ...
        finally { Marshal.FreeHGlobal(pidPtr); Marshal.FreeHGlobal(vidPtr); Marshal.FreeHGlobal(cidPtr); }
    }
    ...
    public Task SaveChapterAsync(string projectId, string volumeId, string chapterId, string plainText)
    {
        ...
        finally { Marshal.FreeHGlobal(pidPtr); Marshal.FreeHGlobal(vidPtr); Marshal.FreeHGlobal(cidPtr); Marshal.FreeHGlobal(contentPtr); }
    }
    ...
    public Task<VolumeSummary> CreateVolumeAsync(string projectId, string name)
    {
        ...
        finally { Marshal.FreeHGlobal(pidPtr); Marshal.FreeHGlobal(namePtr); }
    }
    ...
    public Task<ChapterSummary> CreateChapterAsync(string projectId, string volumeId, string title)
    {
        ...
        finally { Marshal.FreeHGlobal(pidPtr); Marshal.FreeHGlobal(vidPtr); Marshal.FreeHGlobal(titlePtr); }
    }
    ...
    public Task SaveSettingsAsync(LocalSettings settings)
    {
        ...
        finally { Marshal.FreeHGlobal(jsonPtr); }
    }

    public int CalculateWordCount(string text)
    {
        var textPtr = ToUtf8(text);
        try { return writer_core_calculate_word_count(textPtr); }
        finally { Marshal.FreeHGlobal(textPtr); }
    }
    ...
    public string? GetEditorVisualTransaction(string oldText, string newText, string oldSelection, string newSelection)
    {
        ...
        finally { Marshal.FreeHGlobal(oldTextPtr); Marshal.FreeHGlobal(newTextPtr); Marshal.FreeHGlobal(oldSelPtr); Marshal.FreeHGlobal(newSelPtr); }
    }
}
