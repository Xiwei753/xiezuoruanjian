...
//   5. open 桥接：coordinator.open(targetId, text) 不接收 isPersistent 参数（Harmony 无 persistent/temporary
//      两套宿主生命周期），bridge.create 也不接收（Core 的 text_edit_session_open 已是 3 参数）。
...
await testAsync('open(targetId, text) 不再把 isPersistent 传给 bridge.create（Core 3 参数）', async () => {
  const createCalls = []
  const bridge = {
  ...
  async function open(targetId, initialText) {
    // Issue #653 评论 5608056955：open 不再接收 isPersistent，bridge.create 也不接收。
  ...
  const result = await open('c1', 'hello')
...
await testAsync('open(targetId, text) 短文本草稿会话同样走 bridge.create', async () => {
  const createCalls = []
  const bridge = {
