...
pub fn sujian_item_dpr(item_ptr: *mut std::ffi::c_void) -> f64 {
    // SAFETY: item_ptr is a QQuickItem* from Qt's ownership, valid during the
    // item's lifetime. Only reads devicePixelRatio — no mutation or storage.
...
pub fn sujian_create_painter_scaled(image: &mut qmetaobject::QImage, dpr: f64) -> *mut QPainter {
    // SAFETY: img_ptr is a QImage* borrowed from the caller's paint scope.
    // QPainter is heap-allocated and returned to the caller, who must call
    // sujian_delete_painter to free it. The painter borrows the QImage —
    // caller must ensure the QImage outlives the painter.
...
pub fn sujian_delete_painter(painter: *mut QPainter) {
    // SAFETY: painter was heap-allocated by sujian_create_painter_scaled.
    // Caller transfers ownership here — must not use painter after this call.
