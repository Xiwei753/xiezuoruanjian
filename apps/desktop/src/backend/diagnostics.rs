...
fn redact(message: &str) -> String {
    ...
    if let Ok(re_ssh_key) = regex::Regex::new(r"(?i)ssh_private_key\s*[:=]\s*[\s\S]*?-----END[^\n]*PRIVATE KEY-----") {
        result = re_ssh_key.replace_all(&result, "ssh_private_key=[REDACTED]").to_string();
    }

    // PEM private key blocks
    if let Ok(re_pem) = regex::Regex::new(r"-----BEGIN[^\n]*PRIVATE KEY-----[\s\S]*?-----END[^\n]*PRIVATE KEY-----") {
    ...
    if let Ok(re_bearer_header) = regex::Regex::new(r"(?i)\b(authorization)\s*[:=]\s*Bearer\s+\S+") {
        result = re_bearer_header.replace_all(&result, "Authorization: Bearer [REDACTED]").to_string();
    }

    // Sensitive key=value pairs (token, password, secret, etc.)
    if let Ok(re_sensitive_kv) = regex::Regex::new(r#"(?i)\b(token|access_token|refresh_token|authorization|password|passwd|secret|private_key)\s*[:=]\s*(?:"[^"]*"|\S+)"#) {
        result = re_sensitive_kv.replace_all(&result, "$1=[REDACTED]").to_string();
    }

    // Content/body/chapter key=value pairs (user content)
    if let Ok(re_content_kv) = regex::Regex::new(r#"(?i)\b(content|text|body|chapter|chapter_content|chapterContent)\s*[:=]\s*(?:"[^"]*"|[^,}\]\n]+)"#) {
        result = re_content_kv.replace_all(&result, "$1=[REDACTED]").to_string();
    }

    // Bearer tokens standalone
    if let Ok(re_bearer_standalone) = regex::Regex::new(r"(?i)Bearer\s+[A-Za-z0-9\-._~+/]+=*") {
        result = re_bearer_standalone.replace_all(&result, "Bearer [REDACTED]").to_string();
    }
