...
fn redact(message: &str) -> String {
    ...
    // GitHub PAT patterns
    if let Ok(re) = regex::Regex::new(r"ghp_[A-Za-z0-9]{36}") { result = re.replace_all(&result, "[REDACTED]").to_string(); }

    if let Ok(re) = regex::Regex::new(r"gho_[A-Za-z0-9]{36}") { result = re.replace_all(&result, "[REDACTED]").to_string(); }

    if let Ok(re) = regex::Regex::new(r"github_pat_[A-Za-z0-9_]{82}") { result = re.replace_all(&result, "[REDACTED]").to_string(); }
