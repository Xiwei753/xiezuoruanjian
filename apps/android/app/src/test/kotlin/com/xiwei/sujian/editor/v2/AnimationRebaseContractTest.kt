...
class AnimationRebaseContractTest {
    ...
    private fun invokeCompatibleRebaseRoles(planner: AndroidVisualPlanner, role: SliceRole): Set<SliceRole> {
        return planner.rebasePlanner.compatibleRebaseRoles(role)
    }
    ...
    fun deleteRebaseContinuesFromCurrentAlphaToZero() {
        ...
        val rebased = planner.rebasePlanner.applyRebaseState(newSlice, rebaseState)
    ...
    fun survivingInsertSliceContinuesFadeIn() {
        ...
        val planner = AndroidVisualPlanner()
        val method = planner.rebasePlanner.javaClass.getDeclaredMethod(
            "applyRebaseToSlices",
            List::class.java,
        ...
        val result = method.invoke(planner.rebasePlanner, newSlices, rebaseSnapshot, emptyMap<Long, AndroidLineSnapshot>())
            as List<*>
    ...
    fun survivingMoveSliceContinuesToDestination() {
        ...
        val newSlices = listOf<PreparedVisualTransaction.AnimatedSlice>()
        val result = method.invoke(planner.rebasePlanner, newSlices, rebaseSnapshot, snapshotLookup)
            as List<*>
    ...
    fun paragraphIdAlignmentIncludesExtraLinesWhenParagraphGrows() {
        ...
        val planner = AndroidVisualPlanner()
        val result = planner.affectedLayoutPlanner.computeAffectedLines(visualIntent, oldRev, newRev)
    ...
    fun hardBreakInsertIncludesSplitParagraphInAffectedLines() {
        ...
        val planner = AndroidVisualPlanner()
        val method = planner.affectedLayoutPlanner.javaClass.getDeclaredMethod(
        ...
        method.isAccessible = true
        val result = method.invoke(planner.affectedLayoutPlanner, visualIntent, oldRev, newRev)
    ...
    fun blockShiftRebaseContinuesFromCurrentTranslateY() {
        ...
        val planner = AndroidVisualPlanner()
        val method = planner.blockShiftPlanner.javaClass.getDeclaredMethod(
        ...
        val result = method.invoke(planner.blockShiftPlanner, listOf(newShift), rebaseSnapshot, null, null) as List<*>
    ...
    fun blockShiftRebaseWithMergedShiftUsesBestOverlap() {
        ...
        method.isAccessible = true
        @Suppress("UNCHECKED_CAST")
        val result = method.invoke(planner.blockShiftPlanner, listOf(newMergedShift), rebaseSnapshot, null, null) as List<*>
        val rebasedShift = result[0] as PreparedVisualTransaction.BlockShift
    ...
    fun blockShiftMergeWithSlightlyDifferentDeltaY() {
        ...
        method.isAccessible = true
        @Suppress("UNCHECKED_CAST")
        val merged = method.invoke(planner.affectedLayoutPlanner, shifts) as List<*>
    ...
    fun blockShiftRebaseOneToOneMatchingPreventsDuplicateReuse() {
        ...
        method.isAccessible = true
        @Suppress("UNCHECKED_CAST")
        val result = method.invoke(planner.blockShiftPlanner, listOf(newShift1, newShift2), rebaseSnapshot, null, null) as List<*>
    ...
    fun blockShiftRebaseUsesReverseMapperWhenForwardFails() {
        ...
        @Suppress("UNCHECKED_CAST")
        val result = method.invoke(planner.blockShiftPlanner, newBlockShifts, rebaseSnapshot, forwardMapper, reverseMapper)
            as List<*>
    ...
    fun blockShiftRebaseUsesForwardNearMatchWhenExactFails() {
        ...
        @Suppress("UNCHECKED_CAST")
        val result = method.invoke(planner.blockShiftPlanner, newBlockShifts, rebaseSnapshot, offsetMapper, reverseMapper)
            as List<*>
    ...
    fun blockShiftRebase_endUtf8ExclusiveValidatesForwardMatch() {
        ...
        method.isAccessible = true
        @Suppress("UNCHECKED_CAST")
        val result = method.invoke(planner.blockShiftPlanner, listOf(newShift), rebaseSnapshot, offsetMapper, null) as List<*>
        val rebasedShift = result[0] as PreparedVisualTransaction.BlockShift
    ...
    @Test
    fun lineReflow_monotonicMatching_noCrossPairing() {
        val oldSnapshot = makeSnapshotWithClusters(1L, 0, 0, 30, listOf(
            LineClusterSnapshot(clusterId = 0, documentByteStart = 0, documentByteEndExclusive = 10,
                documentUtf16Start = 0, documentUtf16EndExclusive = 10,
                sourceRectInLineImage = android.graphics.Rect(0, 0, 50, 20),
            ...
            LineClusterSnapshot(clusterId = 2, documentByteStart = 20, documentByteEndExclusive = 30,
                ...
                sourceRectInLineImage = android.graphics.Rect(100, 0, 150, 20),
                visualRectInDocument = android.graphics.RectF(100f, 0f, 150f, 20f),
                shapingFingerprint = "fp_c", shapingIdentityConfident = true)
        ))
        val newSnapshot = makeSnapshotWithClusters(2L, 0, 0, 30, listOf(
            LineClusterSnapshot(clusterId = 0, documentByteStart = 0, documentByteEndExclusive = 10,
                documentUtf16Start = 0, documentUtf16EndExclusive = 10,
        ...
        val moveSlices = transaction.animatedSlices.filter { it.role == SliceRole.Move }
        val firstSlice = moveSlices.find { it.clusterByteStart == 10 }
        val secondSlice = moveSlices.find { it.clusterByteStart == 20 }
        assertNotNull("Old cluster at 10 must match new cluster at 10 (not 20) via monotonic constraint", firstSlice)
        assertNotNull("Old cluster at 20 must match new cluster at 20 (not 10) via monotonic constraint", secondSlice)
        if (firstSlice != null) {
            assertEquals("Monotonic: old byte 10 must map to new byte 10, not cross to 20",
                10, firstSlice.clusterByteStart)
        }
    }

    @Test
    fun planClusterReplaceAnimation_monotonicMatching_duplicateFingerprintNoCrossPairing() {
        val oldSnapshot = makeSnapshotWithClusters(1L, 0, 0, 20, listOf(
            LineClusterSnapshot(clusterId = 0, documentByteStart = 0, documentByteEndExclusive = 10,
                documentUtf16Start = 0, documentUtf16EndExclusive = 10,
                sourceRectInLineImage = android.graphics.Rect(0, 0, 50, 20),
        ...
        val moveSlice10 = transaction.animatedSlices.find {
            it.role == SliceRole.Move && it.clusterByteStart == 10
        }
        assertNotNull("Duplicate fingerprint: old byte 10 must match new byte 10, not cross to 0", moveSlice10)
    }

    @Test
    fun preeditUnderline_usesGetSelectionPathPerLineAndClipPath() {
        val renderer = com.xiwei.sujian.editor.v2.render.AndroidTextRenderer()
        val method = renderer.javaClass.getDeclaredMethod(
            "drawPreeditUnderlineUnshifted",
            Canvas::class.java,
        ...
        val sourceFile = java.io.File(
            "/home/xiwei/xiezuoruanjian/apps/android/app/src/main/kotlin/com/xiwei/sujian/editor/v2/render/AndroidTextRenderer.kt"
        )
        if (sourceFile.exists()) {
            val source = sourceFile.readText()
            assertTrue("drawPreeditUnderlineUnshifted must use clipPath for bidi-safe clipping",
                source.contains("canvas.clipPath(path)"))
            assertFalse("drawPreeditUnderlineUnshifted must NOT use computeBounds to collapse path",
                source.contains("computeBounds"))
            assertTrue("drawPreeditUnderlineUnshifted must call getSelectionPath per line",
                source.contains("layout.getSelectionPath("))
        }
    }
}
