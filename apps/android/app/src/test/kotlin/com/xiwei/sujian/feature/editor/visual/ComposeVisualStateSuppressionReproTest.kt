...
class ComposeVisualStateSuppressionReproTest {
    @Test
    fun state_exposes_hiddenRanges_not_suppressedCurrentRanges() {
        ...
        assertTrue("state 应有 hiddenRanges（新 API）", fields.contains("_hiddenRanges"))
        assertFalse("state 不应有 suppressedCurrentRanges（旧 API）", fields.contains("suppressedCurrentRanges"))
    }

    @Test
    fun emptyState_hiddenRanges_empty() {
        val state = ComposeEditorVisualState(targetId = "test-state-suppression-empty")
        assertTrue("空状态 hiddenRanges 应为空", state.hiddenRanges.value.isEmpty())
    }
}
