...
class CoreSettingsEventsTest {
    ...
    private val settingsEnvelope: ResultEnvelope<Unit> = ResultEnvelope(
        success = true,
        changedEntities = listOf(ChangedEntity("SettingsSaved", ""))
    )

    private val otherEnvelope: ResultEnvelope<Unit> = ResultEnvelope(
        success = true,
        changedEntities = listOf(ChangedEntity("OtherEntity", ""))
    )

    @Test
    fun settingsChanged_emitsOnRecord() = runTest {
    ...
    @Test
    fun editorSettingsChanged_emitsOnRecord() = runTest {
        val received = mutableListOf<Unit>()
        val job = launch(UnconfinedTestDispatcher(testScheduler)) {
    ...
    @Test
    fun markEditorChanged_emitsBothFlows() = runTest {
    ...
    @Test
    fun record_nonSettingsEntity_doesNotEmit() = runTest {
        val received = mutableListOf<Unit>()
        val job = launch(UnconfinedTestDispatcher(testScheduler)) {
            CoreSettingsEvents.settingsChanged.collect { received.add(it) }
        }
