...
class AppSyncSettingsViewModelTest {
    ...
    fun `project and app sync config are independent`() {
        val vm = createVm()
        val projectConfig = com.xiwei.sujian.model.SyncConfig(enabled = true, remoteUrl = PROJECT_REMOTE_URL)
        val appConfig = com.xiwei.sujian.model.SyncConfig(enabled = false, remoteUrl = APP_REMOTE_URL)
    ...
    fun `UpdateProjectSyncConfig does not touch appSyncConfig`() {
        val vm = createVm()
        val appConfig = com.xiwei.sujian.model.SyncConfig(enabled = true, remoteUrl = APP_REMOTE_URL)
        vm.handleIntent(SettingsIntent.UpdateAppSyncConfig(appConfig))
        val projectConfig = com.xiwei.sujian.model.SyncConfig(enabled = false, remoteUrl = PROJECT_REMOTE_URL)
        vm.handleIntent(SettingsIntent.UpdateProjectSyncConfig(projectConfig))
        // app config 保持不变
