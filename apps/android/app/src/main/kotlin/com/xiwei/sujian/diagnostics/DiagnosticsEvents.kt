...
object DiagnosticsEvents {
    ...
    fun record(eventType: String, vararg fields: Pair<String, Any?>) {
        val event = linkedMapOf<String, Any?>("event" to eventType).apply {
            for ((k, v) in fields) {
                if (v != null) put(k, v)
            }
    ...
    fun chapterLoad(projectId: String, chapterId: String, byteLength: Int, result: String) =
        record("chapter.load", "projectId" to projectId, "chapterId" to chapterId, "bytes" to byteLength, "result" to result)

    fun chapterSave(projectId: String, chapterId: String, byteLength: Int, result: String, elapsedMs: Long) =
        record("chapter.save", "projectId" to projectId, "chapterId" to chapterId, "bytes" to byteLength, "result" to result, "elapsedMs" to elapsedMs)

    // ── 焦点 / InputConnection ───────────────────────────────────

    fun editorFocus(focused: Boolean) = record("editor.focus", "focused" to focused)

    fun inputConnection(created: Boolean, sessionBound: Boolean) =
        record("editor.input_connection", "created" to created, "sessionBound" to sessionBound)

    // ── IME composition 生命周期 ─────────────────────────────────

    fun compositionBegin(byteStart: Int, byteEndExclusive: Int) =
        record("editor.composition.begin", "start" to byteStart, "end" to byteEndExclusive)

    fun compositionUpdate(preeditBytes: Int, cursorOffset: Int) =
        record("editor.composition.update", "preeditBytes" to preeditBytes, "cursorOffset" to cursorOffset)

    fun compositionCommit(byteStart: Int, byteEndExclusive: Int, committedBytes: Int) =
        record("editor.composition.commit", "start" to byteStart, "end" to byteEndExclusive, "committedBytes" to committedBytes)

    fun compositionCancel(byteStart: Int, byteEndExclusive: Int, preeditBytes: Int) =
        record("editor.composition.cancel", "start" to byteStart, "end" to byteEndExclusive, "preeditBytes" to preeditBytes)
    ...
    fun sessionLifecycle(sessionId: String, action: String) =
        record("editor.session", "session" to sessionId, "action" to action)

    // ── 动画事务 ─────────────────────────────────────────────────

    fun animationStart(transactionId: Long, kind: String, durationMs: Long) =
        record("editor.animation.start", "transaction" to transactionId, "kind" to kind, "durationMs" to durationMs)

    fun animationRebase(transactionId: Long, newTransactionId: Long) =
    ...
    fun animationCancel(transactionId: Long) =
        record("editor.animation.cancel", "transaction" to transactionId)

    fun animationComplete(transactionId: Long, elapsedMs: Long) =
    ...
    fun settingsSaved(field: String, result: String) =
        record("settings.save", "field" to field, "result" to result)

    // ── 同步 ─────────────────────────────────────────────────────

    fun syncEvent(action: String, status: String, detail: String? = null) =
