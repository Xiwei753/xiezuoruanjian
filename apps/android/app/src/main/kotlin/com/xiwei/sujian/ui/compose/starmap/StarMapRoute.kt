...
private fun StarMapEditorScreen(
                ...
                try {
                    ...
                    when (val result = repo.getStarmapPhasedSnapshot(starmapId)) {
                        is BridgeResult.Success -> {
                            ...
                            val edgeRenders =
                                when (val er = repo.computeEdgeRenders(graphData)) {
        ...
        return try {
            ...
            when (result) {
                ...
                is BridgeResult.Error -> {
                    editorState =
                        editorState.copy(
                            lastError = context.getString(R.string.starmap_op_failed, label, result.message),
                ...
                BridgeResult.NotLoaded -> {
                    editorState =
                        editorState.copy(
                            lastError = context.getString(R.string.starmap_op_failed_not_loaded, label),
                            operationInProgress = false,
                        )
                    false
                }
            ...
            editorState =
                editorState.copy(
                    lastError = context.getString(R.string.starmap_op_exception, label, e.message),
                    operationInProgress = false,
                )
            false
        }
        ...
        if (editorState.hasPendingLayoutSave && editorState.starMapData != null) {
            withContext(Dispatchers.IO) {
                try {
                        ...
                        is BridgeResult.Error -> {
                            editorState =
                                editorState.copy(
                                    layoutSaveError =
                                        context.getString(
                                            R.string.starmap_layout_save_failed, result.message,
                                        ),
                                )
                        }
                        BridgeResult.NotLoaded -> {
                            editorState =
                                editorState.copy(
                                    layoutSaveError =
                                        context.getString(R.string.starmap_layout_save_failed_not_loaded),
                                )
                        }
                    }
                } catch (e: Exception) {
                    editorState =
                        editorState.copy(
                            layoutSaveError = context.getString(R.string.starmap_layout_save_exception, e.message),
                        )
                }
            }
        }
        if (editorState.hasPendingViewportSave && editorState.starMapData != null) {
            withContext(Dispatchers.IO) {
                try {
                        ...
                        is BridgeResult.Error -> {
                            editorState =
                                editorState.copy(
                                    viewportSaveError =
                                        context.getString(
                                            R.string.starmap_viewport_save_failed, result.message,
                                        ),
                                )
                        }
                        BridgeResult.NotLoaded -> {
                            editorState =
                                editorState.copy(
                                    viewportSaveError =
                                        context.getString(R.string.starmap_viewport_save_failed_not_loaded),
                                )
                        }
                    }
                } catch (e: Exception) {
                    editorState =
                        editorState.copy(
                            viewportSaveError = context.getString(R.string.starmap_viewport_save_exception, e.message),
                        )
                }
            }
        }
    ...
    StarMapEditorContent(
        ...
        onNodeDrag = { nodeId, x, y ->
            coroutineScope.launch {
                val updatedNodes =
                    editorState.starMapData!!.layout.nodes.map {
                        ...
                        try {
                                ...
                                BridgeResult.NotLoaded -> {
                                    editorState =
                                        editorState.copy(
                                            viewportSaveError =
                                                context.getString(
                                                    R.string.starmap_viewport_save_failed_not_loaded,
                                                ),
                                        )
                                }
                            }
                            ...
                            editorState =
                                editorState.copy(
                                    viewportSaveError =
                                        context.getString(
                                            R.string.starmap_viewport_save_exception, e.message,
                                        ),
                                )
                        }
                    }
            ...
            if (graphNode != null) {
                ...
                target.onCommit = { finalText ->
                    if (finalText.isNotBlank() && finalText.trim() != graphNode.title) {
                        coroutineScope.launch {
                            val ok =
                                executeOperation(context.getString(R.string.starmap_update_node)) {
        ...
        if (graphNode != null) {
            NodeEditPanel(
                ...
                onDelete = {
                    coroutineScope.launch {
                        val ok =
                            executeOperation(context.getString(R.string.starmap_delete_node)) {
    ...
    if (showAddNodeDialog) {
        StarMapAddNodeDialog(
            ...
            onConfirm = { title, kind ->
                coroutineScope.launch {
                    val ok =
                        executeOperation(context.getString(R.string.starmap_add_node)) {
    ...
    if (showAddEdgeDialog && editorState.starMapData != null) {
        StarMapAddEdgeDialog(
            ...
            onConfirm = { fromNodeId, toNodeId ->
                coroutineScope.launch {
                    val ok =
                        executeOperation(context.getString(R.string.starmap_add_edge)) {
