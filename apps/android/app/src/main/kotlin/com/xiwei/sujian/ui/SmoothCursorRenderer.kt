...
class SmoothCursorRenderer(private val editText: WriterEditText) : EditorAnimationRuntime.Animatable {
    ...
    private var lastSetEnabled: Boolean = false
    private var lastSetDurationMs: Long = 0L
    ...
    internal data class CursorTargetCoords(val x: Float, val top: Float, val bottom: Float)

    internal fun computeCursorTarget(pos: Int): CursorTargetCoords {
    ...
    fun setSmoothCursorEnabled(enabled: Boolean, durationMs: Long = 80L) {
        // 重复设置相同值不打断正在进行的动画
        if (enabled == lastSetEnabled && durationMs == lastSetDurationMs) {
            DiagnosticsLogger.d(TAG, "setSmoothCursorEnabled: skipped (same values) enabled=$enabled, durationMs=$durationMs")
            return
        }
        val wasEnabled = smoothCursorEnabled
        lastSetEnabled = enabled
        lastSetDurationMs = durationMs
        smoothCursorEnabled = enabled
        smoothCursorDurationMs = durationMs.coerceIn(30L, 1000L)
        ...
        if (enabled && editText.isFocused) {
            startCursorBlink()
            // 只在从 false->true 启用时或 layout 未就绪时调用 updateCursorTarget(false)
            // 普通重复设置不打断动画
            if (!wasEnabled) {
