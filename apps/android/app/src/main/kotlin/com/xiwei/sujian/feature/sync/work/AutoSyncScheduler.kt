 ...
 * 全量自动同步调度器。
 *
 * 只维护一个周期任务；AutoSyncWorker 读取全局 profile 并执行一次全量同步。
 * 周期任务只负责定期唤醒，实际是否到点由 Worker 根据 syncIntervalSeconds 判断。
 */
class AutoSyncScheduler(context: Context) {
    ...
    fun start() {
        ...
        SyncDiagnosticsEvents.syncEvent(DiagnosticOriginDto.APP, "scheduler", "start")
        scheduleFromSettings(appContext)
        enqueueForegroundCheck(appContext)
    }

    fun stop() {
        SyncDiagnosticsEvents.syncEvent(DiagnosticOriginDto.APP, "scheduler", "stop")
    ...
    companion object {
        ...
        /**
         * 调度单一周期任务，Worker 内部读一份全局 profile 后调用一次 runFullSync。
         *
