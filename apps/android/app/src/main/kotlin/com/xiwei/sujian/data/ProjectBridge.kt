...
class ProjectBridge internal constructor(private val holder: WriterAppServiceHolder) {
    fun listProjects(): BridgeResult<List<Project>> = holder.wrapResult {
        holder.service.listProjects().map { it.toModel() }
    }

    fun createProject(title: String): BridgeResult<Project> = holder.wrapResult {
    ...
    fun renameProject(projectId: String, newTitle: String): BridgeResult<Boolean> = holder.wrapResult {
        holder.service.renameProject(projectId, newTitle)
    }

    fun deleteProject(projectId: String): BridgeResult<Boolean> = holder.wrapResult {
        holder.service.deleteProject(projectId)
    }

    fun reorderProjects(orderedIds: List<String>): BridgeResult<Boolean> = holder.wrapResult {
    ...
    fun flushRecentEdits(): BridgeResult<Boolean> = holder.wrapResult {
        holder.service.flushRecentEdits()
    }

    fun listVolumes(projectId: String): BridgeResult<List<Volume>> = holder.wrapResult {
        holder.service.listVolumes(projectId).map { it.toModel() }
    }

    fun createVolume(projectId: String, title: String): BridgeResult<Volume> = holder.wrapResult {
        holder.service.createVolume(projectId, title).toModel()
    }

    fun renameVolume(projectId: String, volumeId: String, newTitle: String): BridgeResult<Boolean> = holder.wrapResult {
        holder.service.renameVolume(projectId, volumeId, newTitle)
    }

    fun deleteVolume(projectId: String, volumeId: String): BridgeResult<Boolean> = holder.wrapResult {
        holder.service.deleteVolume(projectId, volumeId)
    }

    fun reorderVolumes(projectId: String, orderedIds: List<String>): BridgeResult<Boolean> = holder.wrapResult {
