...
class EditorWindowHost(
    ...
    /** 真正的 session owner — target/session 命令、undo/redo、save/close、外部事实同步。 */
    val sessionHost: EditorSessionHost = EditorSessionHost(sessionCoordinator, appServiceBridge)

    /** 窗口标识 — 同一窗口内的 Compose onDispose 用它调用 detachWindowBinding。 */
    ...
    /**
     * 转发 [EditorSessionHost.authoritativeEditorSnapshots] —
    ...
    /**
     * 业务级关闭 — 由 workspace 导航事件调用。委托给 [sessionHost]。
     */
    fun closeTarget(
        targetId: String,
    ...
    /**
     * 排版设置持续应用 — 窗口层唯一设置写入点（诊断 + 透传 Compose 层）。
     * 排版由 Compose [BasicTextField] 直接应用，本方法只做诊断记录。
     */
    fun applyEditorTypography(
        ...
        DiagnosticsEventsInterop.editorTypography(
    ...
    /** 开始编辑 — 委托给 [sessionHost] 预准备 Rust session。 */
    fun beginEdit(
        targetId: String,
