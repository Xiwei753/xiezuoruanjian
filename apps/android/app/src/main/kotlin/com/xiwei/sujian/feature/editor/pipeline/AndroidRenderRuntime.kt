...
import com.xiwei.sujian.feature.editor.render.AndroidTextRenderer
import com.xiwei.sujian.feature.editor.render.ComposedFrame
import com.xiwei.sujian.feature.editor.render.EditorFrameComposer
...
class AndroidRenderRuntime(
    ...
    fun drawFromFrameState(
        canvas: Canvas,
        frameState: FrameState,
    ) {
        val input = frameState.renderInput
        val effectiveSelStart =
            if (input.selectionAllowed) {
                input.layoutRevision?.selectionStartUtf16 ?: input.selectionStartUtf16
            } else {
                input.cursorUtf16
            }
        val effectiveSelEnd =
            if (input.selectionAllowed) {
                input.layoutRevision?.selectionEndUtf16 ?: input.selectionEndUtf16
            } else {
                input.cursorUtf16
            }
    ...
    fun renderComposedFrame(
        ...
        if (transaction != null && (transaction.animatedSlices.isNotEmpty() || transaction.blockShifts.isNotEmpty())) {
            textRenderer.drawSearchHighlights(
                canvas,
                layout,
                frame.searchHighlightsUtf16,
                frame.blockShifts,
                frame.progress,
            )
            textRenderer.drawSelectionHighlight(
                canvas,
                layout,
                frame.selectionStartUtf16,
                frame.selectionEndUtf16,
                frame.blockShifts,
                frame.progress,
            )
            ...
            textRenderer.drawPreeditUnderline(
                canvas,
                layout,
                frame.compositionStartUtf16,
                frame.compositionEndUtf16,
                frame.blockShifts,
                frame.progress,
            )
            ...
            if (ct != null && ct.shouldAnimate) {
                animationRenderer.drawAnimatedCursor(
                    canvas,
                    transaction,
                    frame.cursorProgress ?: frame.progress,
                    textRenderer.getCursorPaint(),
                )
            } else {
    ...
    companion object {
        ...
        fun shouldDrawAnimatedCursorOnStaticPath(
            cursorTransition: com.xiwei.sujian.feature.editor.visual.PreparedVisualTransaction.CursorTransition?,
