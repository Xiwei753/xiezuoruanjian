...
class ReadableMirrorPublisher(
    ...
    private suspend fun recoverPromotePhase(
        ...
        for ((key, item) in currentItems.toMap()) {
            ...
            // 否则不要用未知内容去生成 manifest。公共镜像在 Download 里，
            // 用户/Provider 都可能让同路径文件内容变化；路径存在不等于它还是本事务那份正文。
            if ((item.state == PendingItem.STATE_PROMOTED || item.state == PendingItem.STATE_COMMITTED) &&
                ...
                when (promotedLookup) {
                    ...
                    is MirrorLookupResult.Missing -> {
                        // promotedRef 指向的文件不存在，不继续，保留 journal 停止
                        DiagnosticsLogger.w(
                            TAG,
            ...
            if (staged == null) {
                // #649 评论 5574521549 问题 4：stagedRef 缺失是事务状态不完整，
                // 不能 continue 静默漏掉章节后继续提交 manifest。旧实现 `continue` 会
                // 跳过该章节，恢复时漏掉它继续 promote 其他章节并提交 manifest，
                // 导致事务结果不完整（该章节既没 promote 也没 rollback）。
                // 改成停止恢复并保留 journal（rollback + return），让下次恢复重试或人工介入。
                // 走到这里的不可能是 PROMOTED/COMMITTED + promotedRef != null（前面已 continue），
                // 对其他正向状态（STAGED/BACKUP_READY/OLD_VACATED），stagedRef 缺失直接停止恢复。
                DiagnosticsLogger.w(
                    TAG,
        ...
        // putChapterEntries / addPublishedProjectId）。
        // 断电窗口：死在 journal 已落盘、stateStore 未更新之间，磁盘 journal 是 PHASE_CLEANUP，
        // 下次恢复走 recoverCleanupPhase 幂等补写 stateStore，不会重复 promote。
        // #649 评论 5576949398 问题 2：从 manifestResult.committedJournal.copy(phase=PHASE_CLEANUP, ...)
        // 继续推进，不再从 manifest 调用之前的旧 journal 重建 cleanup journal。
        ...
        if (!persistPendingJournal(cleanupJournal)) {
    ...
    private suspend fun recoverCleanupPhase(
        ...
        when (journal.transactionType) {
            MirrorTransactionType.UPSERT_PROJECT -> {
                ...
                // 先幂等补写 committed baseline，失败则保留 journal 停止，不继续改 chapter entries。
                if (journal.isManifestCommitted && !persistCommittedBaselineFromJournal(journal)) {
                    DiagnosticsLogger.w(
                        TAG,
                        "Recover cleanup: committed baseline failed for UPSERT_PROJECT " +
            ...
            MirrorTransactionType.DELETE_PROJECT -> {
                ...
                // journal 已落盘且 isManifestCommitted=true 时，committed baseline 可能还没写
                // （断电窗口：journal 落盘后、setCommittedManifest 前）。
                // 先幂等补写 committed baseline，失败则保留 journal 停止，不继续删项目条目。
                if (journal.isManifestCommitted && !persistCommittedBaselineFromJournal(journal)) {
                    DiagnosticsLogger.w(
                        TAG,
                        "Recover cleanup: committed baseline failed for DELETE_PROJECT " +
                            "${journal.projectId}, keeping journal",
                    )
                    return
                }
     ...
     */
    private sealed interface CommittedManifestResolution {
     ...
     */
    private fun resolveCommittedManifestForPublish(storage: ReadableMirrorStorage): CommittedManifestResolution {
    ...
    suspend fun publishProject(projectId: String): MirrorPublishResult {
        try {
            ...
            // （recoverPendingPublishIfNeeded 的 PHASE_STAGE 分支会 storage.rollback(txId) + clearPendingPublish）。
            // 不再把 staging 垃圾残留到 Download/Sujian 且无 txId 可清。
            if (!writePendingPublishJournal(
                    projectId = projectId,
                ...
                DiagnosticsLogger.w(TAG, "Publish project $projectId aborted: PHASE_STAGE journal write failed")
                return MirrorPublishResult.RetryableFailure
            }
            ...
            // 不再把损坏当首次发布；NeedsMigration 触发 ReadableMirrorStateMigration。
            ...
            if (committedManifestResolution is CommittedManifestResolution.Stop) {
                DiagnosticsLogger.w(
                    TAG,
                    "Publish project $projectId aborted: committed manifest corrupted or migration failed",
                )
                return MirrorPublishResult.RetryableFailure
            }
    ...
    suspend fun deleteProject(projectId: String): MirrorPublishResult {
        try {
            ...
            // #649 评论 5576949398 问题 2：从 manifestResult.committedJournal 继续，
            // 不再用 writePendingPublishJournal 从旧字段重建。
            if (!persistPendingJournal(manifestResult.committedJournal)) {
                DiagnosticsLogger.w(
                    TAG,
    ...
    private fun rollbackManifest(
        ...
        // #649 评论 5573750754 修复 3：manifest 子事务从未开始，旧 manifest 完全没动。
        // manifestTargetJson 是"是否开始"的天然标记。只有 manifestTargetJson != null 之后，
        // manifestOldRef == null 才能解释成"本事务开始时确实没有旧 manifest"。
        // 未开始时设备上可能有上一版 manifest，直接 return true，不 lookup final 去和 null hash 比较。
        ...
        // #649 评论 5573750754 修复 4：rollback 必须自己做三态发现（lookupBackup），
        // 不能仅凭 journalContext.manifestBackupRef == null 判定物理 backup 不存在。
        // 存在窗口：prepareBackup() 已把 old manifest move/copy 到 .staging/<tx>/backup，
        // 进程死亡，MANIFEST_BACKUP_READY journal 还没写成功，journal 里 manifestBackupRef 仍 null
        // 但物理 backup 已存在。
        val backup =
                ...
                when {
                    ...
                    finalHash == journalContext.manifestOldContentHash -> {
                        ...
                        if (!writePendingPublishJournal(
                            ...
                            DiagnosticsLogger.w(
                                TAG,
                                "rollback manifest: journal write failed after OLD_RESTORED (final already old)",
                            )
                            return false
                        }
                        return true
            ...
            is MirrorLookupResult.Missing -> {
                ...
                if (journalContext.manifestNewRef != null) {
                    ...
                    if (!writePendingPublishJournal(
                        ...
                        DiagnosticsLogger.w(
                            TAG,
                            "rollback manifest: journal write failed after NEW_REMOVED (final missing)",
                        )
                        return false
                    }
                }
        ...
        // #649 评论 5573750754 修复 4：backup == null 现在表示 lookupBackup 明确返回 Missing
        // （物理 backup 确实不存在）。不能直接 setManifestUri(manifestOldRef.uri) 猜 old 还在 final。
        // 如果 manifestOldRef != null、final 明确 Missing、backup 也明确 Missing，
        // 这是无法恢复的状态，必须保留 journal 停止。
        // 走到这里时 final 上的 new manifest 已被"1."块删除（或 final 本来就 Missing），
        // 再 lookup 一次 final，确认 old manifest 是否还在。
        if (backup == null) {
            val oldStillInFinal = storage.lookup(manifestRelativePath)
            when (oldStillInFinal) {
                is MirrorLookupResult.Found -> {
                    // final 上有文件，校验是否是 old manifest（用冻结的 hash 校验身份）
                    val oldHash = journalContext.manifestOldContentHash
                    if (oldHash != null) {
                        val hashResult = storage.readTextAndHash(oldStillInFinal.ref)
                        if (hashResult != null && hashResult.second == oldHash) {
                            ...
                            if (!writePendingPublishJournal(
                                ...
                                DiagnosticsLogger.w(
                                    TAG,
                                    "rollback manifest: journal write failed after OLD_RESTORED (backup null, old verified)",
                                )
                                return false
                            }
                            return true
     ...
     *
     * 统一 helper，供 publishProject / deleteProject / recoverPromotePhase / recoverCleanupPhase
     * 在 manifest 提交成功后写入 private committed baseline。
     *
     * 校验流程：
     * 1. journal.manifestTargetJson 和 journal.manifestNewContentHash 都非空
     * 2. computeContentHash(json) == hash（hash 自洽）
     * 3. mirrorManifestFromJsonStrict(json) 严格解析通过（schema/字段完整性）
     * 4. stateStore.setCommittedManifest(json, hash) 持久化成功
     *
     * 任一步骤失败返回 false，调用方应停止本轮操作（保留 journal 重试）。
     *
     * @param journal 已推进到 MANIFEST_COMMITTED 的 journal
     * @return true 表示 committed baseline 持久化成功；false 表示失败
     */
    private fun persistCommittedBaselineFromJournal(journal: PendingMirrorPublish): Boolean {
        val json =
     ...
     *
     * #649 评论 5576949398 问题 2：收口成 (committedJournal, newRef) 两个字段。
     * [committedJournal] 是 [publishManifestWithDesiredTransactional] 内部已经推进到
     * [ManifestTransactionState.MANIFEST_COMMITTED] 的最新 currentJournal，
     * 包含 manifestTargetJson、manifestNewContentHash、manifestOldContentHash、
     * manifestOldRef、manifestStagedRef、manifestNewRef、manifestBackupRef、
     * manifestSwapState=MANIFEST_COMMITTED、frozenManifestPlan、frozenManifestPlanHash、
     * 最新 items/newEntries。
     *
     * 调用方（publishProject / deleteProject / recoverPromotePhase）从
     * `manifestResult.committedJournal.copy(phase=PHASE_CLEANUP, ...)` 继续推进，
     * 不再从 manifest 调用之前的旧 journal 重建 cleanup journal。
     *
     * @property committedJournal 已推进到 MANIFEST_COMMITTED 的最新 journal
    ...
    private suspend fun publishManifestWithDesiredTransactional(
                ...
                when (resumeState) {
                    ManifestTransactionState.MANIFEST_COMMITTED -> {
                        ...
                        when (existingFinal) {
                            is MirrorLookupResult.Found -> {
                                ...
                                // 不要把未知 manifest 当成已提交。
                                val committedHashResult = storage.readTextAndHash(existingFinal.ref)
                                ...
                                if (committedHashResult.second != manifestNewContentHash) {
                                    DiagnosticsLogger.w(
                                        TAG,
                                        "Manifest transaction: COMMITTED but final hash mismatch, keeping journal",
                                    )
                                    return null
                                }
                                // #649 评论 5576949398 问题 2：返回 committedJournal（currentJournal）
                                // 已包含 manifestTargetJson/hash/refs/manifestSwapState=MANIFEST_COMMITTED。
                                // 恢复 COMMITTED 路径下 currentJournal 已在入口合并了 items/newEntries，
                                // 但 manifest committed 状态字段可能还来自 journalContext；
                                // 这里补一次 copy 确保 manifestNewRef/manifestSwapState/isManifestCommitted 正确。
                                val committedJournalForCommittedResume =
                                    currentJournal.copy(
                                ...
                                return ManifestTransactionResult(
                                    committedJournal = committedJournalForCommittedResume,
                            ...
                            is MirrorLookupResult.Missing -> {
                                DiagnosticsLogger.w(
                                    TAG,
                                    "Manifest transaction: COMMITTED but final missing, rolling back",
                                )
                                if (staged != null) {
                                    storage.delete(
                                        MirrorFileRef(
                    ...
                    ManifestTransactionState.MANIFEST_PROMOTED -> {
                        ...
                        when (existingFinal) {
                            is MirrorLookupResult.Found -> {
                                // #649 评论 5573750754 修复 5：必须校验 hash == manifestNewContentHash。
                                // hash 不匹配或读取失败就保留 journal，不要把未知 manifest 当成已 promote。
                                val promotedHashResult = storage.readTextAndHash(existingFinal.ref)
                                ...
                                if (promotedHashResult.second != manifestNewContentHash) {
                                    DiagnosticsLogger.w(
                                        TAG,
                                        "Manifest transaction: PROMOTED but final hash mismatch, keeping journal",
                                    )
                                    return null
                                }
                                ...
                                // #649 评论 5576949398 问题 2：返回 committedJournal（currentJournal）
                                // 已推进到 MANIFEST_COMMITTED（上面 copy 已设置 manifestNewRef/manifestBackupRef/
                                // isManifestCommitted/manifestSwapState）。
                                return ManifestTransactionResult(
                                    committedJournal = currentJournal,
                                    newRef = existingFinal.ref,
                                )
                            }
                            is MirrorLookupResult.Missing -> {
        ...
        // #649 评论 5576949398 问题 2：返回 committedJournal（currentJournal）
        // 已推进到 MANIFEST_COMMITTED，包含 manifestTargetJson/manifestNewContentHash/
        // manifestOldContentHash/manifestOldRef/manifestStagedRef/manifestNewRef/
        // manifestBackupRef/frozenManifestPlan/frozenManifestPlanHash/最新 items/newEntries。
        return ManifestTransactionResult(
            committedJournal = currentJournal,
            newRef = newRef!!,
        )
