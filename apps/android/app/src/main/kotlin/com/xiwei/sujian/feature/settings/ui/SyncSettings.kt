 ...
 *
 * 按逻辑分组拆成多张 High 卡（不再按"一个 TextField 一个卡"分）：
 ...
 *
 * 每张内卡内部各自 collect 自己需要的那一个 row state。
 */
@Composable
fun SyncSettingsContent(vm: SettingsViewModel) {
    // ── 同步通用组（说明 + 启用同步 + 自动同步）──
    ...
    val displayResult = structured?.let { resolveStructuredResult(it) }
