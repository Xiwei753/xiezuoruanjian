...
这些边界已有 `tools/check_android_architecture.py` 扫描，不要绕过检查器。
...
- 继续写现有 `feature/editor/` 分层；不要为了一个问题新增旁路 Editor、第二个状态源或窗口覆盖层。

## Rust / UniFFI 边界

生成绑定目录在 `app/build/generated/writer-uniffi/`，不要手改。
...
AI 和非 AI 是独立 flavor：`ai`、`noAi`。AI 专项测试放对应 AI 源集，不要把通用测试复制跑两遍。
...
修改 package、目录或模块依赖时，先看 `tools/check_android_architecture.py` 里已经存在的约束，不要凭感觉新造层级。
