...
fn issue702_design_tokens_consumes_single_theme_state_json() {
    ...
    assert!(src.contains("themeStateJson"), "DesignTokens 应有 themeStateJson 属性");
    assert!(src.contains("_themeState"), "DesignTokens 应从 _themeState 解析");
    assert!(src.contains("_themeState.is_dark"), "isDark 应从 _themeState.is_dark 读取");
    assert!(src.contains("_themeState.scheme"), "scheme 应从 _themeState.scheme 读取");
    assert!(
...
fn issue702_theme_controller_publishes_unified_theme_state_json() {
    ...
    assert!(src.contains("theme_state_json: qt_property!"), "应发布 theme_state_json");
    assert!(src.contains("\"is_dark\""), "theme_state_json 应包含 is_dark");
...
fn issue702_text_visual_operation_kind_no_cursor_variant() {
    ...
    assert!(src.contains("Insert") && src.contains("Delete"), "应包含 Insert/Delete");
    assert!(
        !src.contains("Cursor,\n"),
...
fn issue702_cursor_animation_state_has_own_timeline() {
    ...
    assert!(src.contains("pub struct CursorAnimationState"), "CursorAnimationState 应存在");
    assert!(
        !src.contains("driver_key: VisualTransactionKey"),
        "CursorAnimationState 不应再有 driver_key"
    );
    assert!(src.contains("started_at: Option<Instant>"), "应有 started_at 字段");
    assert!(src.contains("duration_ms: u64"), "应有 duration_ms 字段");
    assert!(src.contains("fn sample_progress"), "应有 sample_progress 方法");
}

#[test]
fn issue702_cursor_transition_tween_has_duration_ms() {
    let src = read_src("src/sujian_editor_item/cursor_animation.rs");
    assert!(src.contains("duration_ms: u64"), "CursorTransition::Tween 应有 duration_ms 字段");
}
...
fn issue702_scene_graph_rebuilds_on_animation_clip() {
    ...
    assert!(src.contains("has_animation_clip"), "应有 has_animation_clip 判断");
    assert!(
        src.contains("needs_relayout || has_animation_clip"),
        "需要在 needs_relayout 或 has_animation_clip 时重建"
    );
    assert!(src.contains("compute_clip_rects_from_patches"), "应有裁剪计算");
}
...
fn issue702_delete_conceal_has_same_frame_progress() {
    let src = read_src("src/sujian_editor_item/animation_coordinator.rs");
    assert!(src.contains("AnimatedSliceKind::DeleteConceal"), "应存在 DeleteConceal");
    assert!(src.contains("InsertReveal"), "应存在 InsertReveal");
    assert!(src.contains("delete_unit_progress"), "应有 delete_unit_progress");
    assert!(src.contains("sampled_rect_at_progress"), "caret track 应用 sampled_rect_at_progress");
}
...
fn issue702_qquickitem_impl_starts_cursor_timeline_on_idle() {
    ...
    assert!(src.contains("CursorSampleOutcome::Idle"), "应处理 Idle 状态");
    assert!(src.contains("started_at = Some(frame_now)"), "Idle 时应用 frame_now");
    assert!(
