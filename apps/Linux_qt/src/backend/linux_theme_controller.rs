...
impl LinuxThemeController {
    ...
    fn resolved_scheme_json(&self) -> QString {
        self.with_app("{}".into(), |app| {
            ...
            let is_dark = Self::compute_is_dark(&appearance_mode, app);
    ...
    fn is_dark(&self) -> bool {
        self.with_app(false, |app| {
            let mode = app.setting_appearance_mode().to_string();
            Self::compute_is_dark(&mode, app)
        })
    }
    ...
    fn compute_is_dark(mode: &str, app: &AppBackend) -> bool {
        match mode {
            "dark" => true,
            "light" => false,
            _ => {
                let theme_mode = app.setting_theme_mode().to_string();
