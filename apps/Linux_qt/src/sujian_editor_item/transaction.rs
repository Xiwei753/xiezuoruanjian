    ...
    pub(crate) fn record_transaction(
        ...
        if self.current_typing_animation_enabled && vt.is_some() && !self.current_is_scrolling {
            if let Some(ref vt) = vt {
                ...
                let old_layout_snapshot = self.layout_snapshot_for_text(&vt.old_text, width);
                let new_layout_snapshot = self.layout_snapshot_for_text(&new.text, width);

                let old_revision = self.layout_revision;
                let new_revision = layout_revision::LayoutRevision::next();

                let (old_snap, new_snap) = LineSnapshotBuilder::build_old_new_snapshots(
                    &old_layout_snapshot,
                    &new_layout_snapshot,
                    old_revision,
                    new_revision,
                ...
                self.previous_layout_snapshot = Some(self.current_layout_snapshot.clone().unwrap_or_else(|| {
                    EditorLayoutSnapshot::new(old_layout_snapshot, Vec::new(), None, crate::editor::layout::CaretAffinity::Downstream)
                }));
                self.current_layout_snapshot = Some(new_snap);

                editor_animation_debug_log(&format!(
                    "record_transaction: processed via line snapshot pipeline, kind={:?}, has_active_insert={}",
                    vt.kind,
