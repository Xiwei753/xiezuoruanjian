...
object WorkspaceManager {
    ...
    fun initWorkspaceIfNeeded(context: Context) {
        ...
        // Ensure required files exist per docs/workspace_format.md
        val manifestFile = File(workspaceDir, "workspace_manifest.json")
        if (!manifestFile.exists()) {
            manifestFile.writeText("{\"version\": 1}")
        }

        val settingsDir = File(workspaceDir, "app-meta/settings")
        if (!settingsDir.exists()) {
            settingsDir.mkdirs()
        }

        val localSettingsFile = File(settingsDir, "settings.local.json")
        if (!localSettingsFile.exists()) {
            localSettingsFile.writeText("{}")
        }
    }
